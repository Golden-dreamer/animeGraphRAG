...
- [configuration.md](./configuration.md) — справочник по параметрам (.env, docker-compose.yml) и API-эндпоинтам
...
Модули:
- `parsers/anime/` — airing-parser (контейнер `airing-parser`, порт 8567)
- `parsers/user_anime/` — user-anime: stats-страницы (контейнер `user-anime`, порт 8568)
- `parsers/user_user/` — user-user: animelist refresh (контейнер `user-user`, порт 8569)
- `parsers/coordinator_app.py` — координатор (контейнер `coordinator`, порт 8570)
- `backend/` — GraphRAG (контейнер `graphrag`, порт 8666)