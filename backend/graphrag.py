...
Сейчас тебе вернулся результат. Сформулируй ответ.

Правила:
1. Отвечай ТОЛЬКО на основе данных из результата запроса. Не придумывай факты.
2. Если результат пустой — скажи честно, что не нашёл информации.
...
6. На языке вопроса пользователя (русский вопрос — русский ответ, английский — английский).
"""
...
_TITLE_PROMPT = """\
Сгенерируй краткое название для чата об аниме на основе вопроса пользователя.
Правила:
1. Максимум 5 слов.
2. Без кавычек, без точки в конце.
3. На языке вопроса (русский вопрос — русское название, английский — английское).
4. Только название, без префиксов типа "Чат:" или "Topic:".
"""


def generate_title(question: str) -> str:
    """Генерирует короткое название чата из первого вопроса пользователя."""
    try:
        title = _llm_call(_TITLE_PROMPT, question, max_tokens=30)
        title = title.strip().strip('"').strip("'").strip(".")
        if title and len(title) <= 60:
            return title
        # Fallback: обрезка вопроса
        return question[:40].strip() + ("..." if len(question) > 40 else "")
    except Exception as e:
        log.warning("Title generation failed: %s — using fallback", e)
        return question[:40].strip() + ("..." if len(question) > 40 else "")
